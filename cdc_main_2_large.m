...
n = 100;   % works for the 3-D case
x1 = 1;  % index for generating posotive elements %%0.22--30
x2 = 0.4; % index for generating negative elements %% 0.08--30


Q_pos_rd = Q_generator(n,x1);
X_neg_rd = Q_neg_generator(n,x2,Q_pos_rd);
Y_neg_rd = Q_neg_generator(n,x2,Q_pos_rd);
Z_neg_rd = Q_neg_generator(n,x2,Q_pos_rd);

X_pos_rd = Q_pos_rd - X_neg_rd;
Y_pos_rd = Q_pos_rd - Y_neg_rd;
Z_pos_rd = Q_pos_rd - Z_neg_rd;
...
c_1 = Gaussian(x, .3, sigma); 
c_2 = Gaussian(x, .5, sigma);
c_3 = Gaussian(x, .7, sigma);
...
[Z_pos, Z_neg, Z] = sign_matrix(X_pos_rd,X_neg_rd);
...
function X_neg_rd = Q_neg_generator(n,x,X_pos_rd)
R = X_pos_rd.*exprnd(x,[n,n,n]);
R(R>1) = 1;
X_neg_rd = round(R);
end
